...
import { getAllProblemByPageAndLimit } from "../../features/Problems/problemsSlice";
import "../../App.css";
const { Option } = Select;
...
const ProblemManagement = () => {
  ...
  const naturalDisasters = useSelector((state) => state.problem?.list);
  const totalPages = useSelector((state) => state.problem?.totalPages);
  const currentPage = useSelector((state) => state.problem?.currentPage);
  ...
  useEffect(() => {
    dispatch(getAllProblemByPageAndLimit({ pagination }));
  }, [dispatch, pagination]);
